		...
		<div id="t1" class="tab-pane active">
			...
			<table class="table table-bordered mb30">
				...
				<tbody id="staff-body">
					...
					<tr>
						<td><a href="/stat/agency/index/tab/0?year=<?= $year ?>&product_id=<?= $agency['product_id'] ?>"><?= $agency['product_name'] ?></a></td>
			...
			<div style="margin: 20px 0 0;">
				刻度单位：
				<a class="btn btn-sm <?= $unit == 1 ? 'btn-primary' : 'btn-default' ?>" href="<?= URL::addParam(URL::full(), 'unit', 1) ?>">日</a><a class="btn btn-sm <?= $unit == 0 ? 'btn-primary' : 'btn-default' ?>" href="<?= URL::addParam(URL::full(), 'unit', 0) ?>">月</a>
			</div>
			<div class="row">
				<div class="col-md-12">
					<!--------ri统计------>
	                		...
	                		var option = {
			                		legend: {
			                			data: [
						                	...
						                	{
				                				name:'<?= mb_substr($agency['product_name'], 0, 12, 'UTF-8') ?>',
			                			...
			                			selected: {
				                			<?php foreach($agencies as $k => $agency) { // 默认显示5个分销商 ?>
			                					'<?= mb_substr($agency['product_name'], 0, 12, 'UTF-8') ?>' : <?= $k < 5 ? 'true' : 'false' ?><?= $k < $countAgencies -1 ? ',' : '' ?>
			                				<?php } ?>
			                			},
			                	        orient : 'vertical',
			                	        x: '0',
