...
class Users extends UActiveRecord {
	...
	public function rules() {
		...
		return array(
			...
			array('repassword', 'compare', 'compareAttribute' => 'password', 'on' => 'create', 'message' => "密码输入不一致"),
			array('mobile', 'match', 'pattern' => "/^1\d{10}$/", 'allowEmpty' => false, 'message' => "手机号码格式不正确"),
			array('account', 'length', 'min' => 3, 'max' => 20),
...
//			array('repassword', 'length', 'is'=>6, 'on'=>'create'),
			...
			array('landscape_id,print_type, status, is_super, is_guide, last_updated_source, is_weixin', 'numerical', 'integerOnly' => true),
			...
			array('id, account, password, password_str, name, sell_role, organization_id, landscape_id,print_type, gender, email, mobile, telephone, birthday, identity, position, biography, status, created_by, created_at, updated_at, deleted_at, is_super, is_guide, last_updated_source, is_weixin', 'safe', 'on' => 'search'),
		);
	}
	...
	public function search() {
		...
		$criteria->compare('password_str', $this->password_str, true);
		$criteria->compare('name', $this->name, true);
		$criteria->compare('sell_role', $this->sell_role, true);
		$criteria->compare('organization_id', $this->organization_id, true);
		$criteria->compare('landscape_id', $this->landscape_id);
