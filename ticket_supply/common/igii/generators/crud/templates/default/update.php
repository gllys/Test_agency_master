...
<div class="tongj_cont clearfix">
    <ul id="op_nav">
        ...
        <li class="ton_now"><a href="/<?php echo $controller_id ?>/update/id/<?php echo "<?php "; ?> echo $_GET['id'] <?php echo "?>"; ?>" title="更新<?php echo $tableName ?>">更新<?php echo $tableName ?> </a></li>
    </ul>
</div>
<!--二级目录 区End-->
                    ...
                    <tr>
                        ...
                        <td class="lie2">
                            ...
                            if ($this->searchTime($item['column_comment'])) { //时间
                                ?>    
                                <input name="<?php echo $item['column_name'] ?>" readonly="readonly" onFocus="WdatePicker({startDate: '%y-%M-01 HH:mm', position: {left: -1, top: -2}, dateFmt: 'yyyy-MM-dd HH:mm', minDate: '%y-%M-%d'})" type="text" class="sp_sxming" value="<?php echo "<?php "; ?> echo date('Y-m-d H:i:s',$model['<?php echo $item['column_name'] ?>']) <?php echo "?>"; ?>" />
                            <?php } else if ($this->searchLog($item['column_comment'])) { //日志 ?>
                                <textarea name="<?php echo $item['column_name'] ?>"><?php echo "<?php "; ?> echo $model['<?php echo $item['column_name'] ?>'] <?php echo "?>"; ?></textarea>
                            <?php } else if ($this->searchFile($item['column_comment'])) { //图片 ?>
                                <input type="hidden" class="sp_sxming" name="<?php echo $item['column_name'] ?>" value="<?php echo "<?php "; ?> echo $model['<?php echo $item['column_name'] ?>'] <?php echo "?>"; ?>" />
                                <button id="<?php echo $item['column_name'] ?>">上传图片</button>
                                <span class="icon_bitianfu">*</span><span class="wanr_txt"></span>
                                <img src="<?php echo "<?php "; ?> echo $model['<?php echo $item['column_name'] ?>'] <?php echo "?>"; ?>" style="vertical-align: middle;" id="<?php echo $item['column_name'] ?>_img" height="100px" />
                            ...
                            <?php } else { //其它   ?>
                                <input type="text" class="sp_sxming" name="<?php echo $item['column_name'] ?>" value="<?php echo "<?php "; ?> echo $model['<?php echo $item['column_name'] ?>'] <?php echo "?>"; ?>"  />
                            <?php } ?>
