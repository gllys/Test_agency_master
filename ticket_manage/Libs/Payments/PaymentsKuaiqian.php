...
final class PaymentsKuaiqian
{
    ...
    public function dopay($payment, $orderInfo, &$msg)
    {
        ...
        //订单名称
        $this->productName = $orderInfo['landscape']['name'].'-'.$orderInfo['ticket']['name'];
        //付款金额
        $this->orderAmount = $payment['money']*100;
        //订单描述
        $this->productDesc = '订单编号:'.$payment['order_id'];
        //订单提交时间
        $this->orderTime = date('YmdHis', strtotime($payment['updated_at']));
        //扩展字段1 格式：参数名1:参数值1;参数名2:参数值2;....
        $this->ext1 = "order_id:{$payment['order_id']};";

        //输出日志
        $this->_log("start \n order_id:{$payment['order_id']} ;payment_id:{$payment['id']}", 'pay');
    ...
    {
        $this->_syncCallback('synccallback', $ret);

        //取得扩展字段"ext1"的值
        $this->_loadReceiveExt($_REQUEST, $ext1);

        //页面重定向
        header("Location:".base_url()."/shopping_succ_{$ext1['order_id']}.html");
    }
    ...
    {
        $this->_syncCallback('asynccallback', $ret);
        //echo "<result>1</result><redirecturl>{$this->pageUrl}</redirec turl>";
        echo "<result>{$ret['result']}</result>";
        exit();
    }
    ...
    {
        loadConfigINI($iniData, PI_PROJECT_ROOT."Libs/Payments/Config.ini");
        ...
        $this->merchantPemFilePath = $iniData['KUAIQIAN']['MERCHANT_PEM_FILE_PATH'];
        $this->merchantPemPassword = $iniData['KUAIQIAN']['MERCHANT_PEM_PASSWORD'];
        $this->billCerFilePath = $iniData['KUAIQIAN']['KUAIQIAN_CER_FILE_PATH'];
        $this->paybackTimeout =  $iniData['KUAIQIAN']['PAY_BACK_TIMEOUT'];

        $this->bgUrl = 'http://'.PI_APP_DOMAIN."/api/{$this->appKey}/async_callback.html";
        $this->pageUrl = 'http://'.PI_APP_DOMAIN."/api/{$this->appKey}/sync_callback.html";
     ...
     * @param unknown $ret
     */
    private function _syncCallback($mode, &$ret = array())
    {
        $ret['result'] = '0';
        ...
        if ($verifyFlg == 1) {
            ...
            switch($_REQUEST['payResult']) {
                case self::PAY_SUCC:
                    //支付成功
                    $ret['result'] = '1';
                    $this->_updatePaymentData($_REQUEST);
                    break;
                default:
                    break;
            }
     ...
     * return bool
     */
    private function _updatePaymentData($ret)
    {
        $paymentsModel  = $this->load->model('payments');
        $oldPaymentInfo = $paymentsModel->getOne('id='.$ret['orderId'], '','order_id,status');
        ...
        $filter = 'id=\''.$ret['orderId'].'\' AND status <> \'succ\'';
    ...
    private function _log($msg, $method)
    {
        ...
        logMsgToFile($this->_getLogPath($method) , $msg);
    }
}
/* End */