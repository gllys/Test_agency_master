...
class ApiModel {
    ...
    public function __call($name, $arguments) {
        ...
        if ($cache && $this->cacheId !== null) {
            ...
            if (empty($data)) {
                ...
                $data = self::curlPost($serverUrl, $params);
                $this->getCache()->set($key, $data, $cacheTime);
            }
            return json_decode($data, true);
        } else {
            $params['sign'] = $this->sign($params, $this->sign);
            $data = $this->curlPost($serverUrl, $params);
            return json_decode($data, true);
        }
    }
     ...
     * @return string 
     */
    public function curlPost($url, $array, $timeout = 5) {
