    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <table class="table table-normal table-1">
                ...
                <tr>
                    <td>绑定供应商</td>
                    <td><?php echo $supply ? "<a href='/scenic/managequip/supply/id/$equipment[id]'>".$supply['name']."</a>"  : '无'; ?></td>
                    <td>绑定景区</td>
                    <td><?php
                        echo $landscape ?"<a href='/scenic/managequip/landscape/id/$equipment[id]'>".$landscape['name']."</a>"  :( $supply ? "<a href='/scenic/managequip/landscape/id/$equipment[id]'>无</a>" : '无'); ?>
                    </td>
                    <td>安装位置</td>
                    <td><?php echo $poi ? "<a href='/scenic/managequip/scenic/id/$equipment[id]'>".$poi['name']."</a>" : '无'; ?></td>
                </tr>
                <tr>
                    <td>更新时间</td>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($item['id'] == $equipment['organization_id']): ?>
                                已绑定
                            <?php else: ?>
                                <a class="clearPart btn btn-primary btn-sm" href="javascript:;" onclick="bindSupply(<?php
                                echo
                                $item['id']
                                ?>, '<?php echo $item['name'] ?>')">绑定
                                </a>
        <?php endif; ?>
                        </td>
                    </tr>
        ...
        PWConfirm('确定绑定 ' + name + ' 供应商吗？', function() {
            $.post('/scenic/managequip/saveESupply', {eid: equipment_id, sid: id}, function(data) {
                if (typeof data.errors != 'undefined') {
                	...
                	setTimeout(function(){
						alert('绑定成功', function(){window.location.reload();});
						},
					1000);
                }
            },
        ...
        $('#btn-remove').click(function() {
            ...
            PWConfirm('解除绑定将把安装景区和安装位置一起解除，您确定解除绑定？', function() {
                $.post('/scenic/managequip/removeESupply', {eid: "<?php echo $equipment['id']; ?>"}, function(data) {
                    ...
                    setTimeout(function(){
						alert('解除绑定成功', function(){window.location.reload();});
						},
					1000);
