...
<div class="contentpanel">
		...
		<div class="panel-body">
			<form action="/ticket/single/" class="form-inline">
				<div class="form-group">
					<select data-placeholder="Choose One" style="width: 100%; padding: 0 10px;" id="distributor-select-search" name="scenic_id">
	...
	<div class="table-responsive">
        <table class="table table-bordered mb30" style="min-width: 1096px;">
			<thead>
				<tr>
					<th width="100">
					...
					<th style="width:55px;">散客价</th>
					<th style="width:55px;">团队价</th>
					<th style="width:80px;">销售起始</th>
					<th style="width:80px;">销售结束</th>
					<th style="width:75px;">日库存设置</th>
					<th>分销策略</th>
			...
			<tbody id="staff-body">
				...
				<tr>
					...
					<td><?php
						//todo optimize
						$rs = Landscape::api()->lists(array('ids' => $item['scenic_id'],'items'=>10000), true);
						$_lists = ApiModel::getLists($rs);
						foreach ($_lists as $v):
						echo isset($v['name']) ? $v['name'].'<br/>' : '';
						endforeach;    
						?>
                    </td>
					<td>
						...
						<?php if ($item['state'] == 1): ?>
						<button class="btn btn-xs btn-success btn-bordered pull-right" style="cursor:auto;">已上架</button>
						<?php else: ?>
						<button class="btn btn-xs btn-danger btn-bordered pull-right" style="cursor:auto;">已下架</button>
						<?php endif; ?>
					</td>
					...
					<td>
						<a style="cursor: pointer; cursor: hand;" data-target=".modal-bank" data-toggle="modal" href="/ticket/single/inventory/?id=<?= $item['id']?>&rid=<?= $item['rule_id']?>&name=<?= $item['name']; ?>" onclick="modal_jump(this);"><?= empty($item['rule_id']) ? '' : '已' ?>设置</a>
					...
					<td>
						<a style="cursor: pointer;cursor: hand;"  data-target=".modal-bank" data-toggle="modal" href="/ticket/single/policy/?id=<?= $item['id'] ?>&policy_id=<?= $item['policy_id'] ?>" onclick="modal_jump(this);"><?= empty($item['policy_name']) ? '设置分销商策略' : $item['policy_name'] ?></a>
						<?php if(!empty($item['policy_name']) || $item['rule_id']>0) {?>
					...
					<td>						
						...
						<?php else: ?>
							<a onclick="up(<?php echo $item['id'] ?>);
								return false;" style="cursor: pointer;cursor: hand;">上架</a>
