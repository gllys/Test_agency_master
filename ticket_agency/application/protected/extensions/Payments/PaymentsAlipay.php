...
final class PaymentsAlipay {
    ...
    //partner
    private $_partner = '2088711559366483';
    //partner
    private $_key = 'mv026mzo61md1v7uxl40uqdymeqnsiq6';
    //卖家账号
    private $seller_email = 'iihuilian2015@163.com';
    ...
    public function dopay($payment, $orderInfo, &$msg) {
        ...
        $this->notify_url = 'http://' . $_SERVER['HTTP_HOST'] . "/order/payments/api/callback/async/way/alipaybill/org_id/" . Yii::app()->user->org_id;
        $this->return_url = 'http://' . $_SERVER['HTTP_HOST'] . "/order/payments/api/callback/sync/way/alipaybill/org_id/" . Yii::app()->user->org_id;
        ...
        //订单名称
        $subject = isset($orderInfo['subject']) ? $orderInfo['subject'] : $orderInfo['landscape']['name'] . '-' . $orderInfo['ticket']['name'];
        //必填
        //付款金额
        $total_fee = $payment['amount'];
        //必填
        //订单描述
        $body = '订单id:' . $payment['order_ids'];
        //商品展示地址
        $show_url = isset($orderInfo['show_url']) ? $orderInfo['show_url'] : Yii::app()->basePath . '/shopping_reserve_' . $orderInfo['ticket']['ticket_template_id'] . '.html';
        ...
        //$logMsg = date('Y-m-d H:i:s')." pay start \n order_id:".$payment['order_ids'].",payment_id:".$payment['id']."\n";
        ...
        if ($this->_getSignVeryfy($arrQueryStrs, $arrQueryStrs['sign'])) {
            ...
            if ($ret['status'] == 'succ') {
                ...
                Payment::api()->update(array(
                    ...
                    'status' => 'succ',
                    'payment' => 'alipay',
                    'distributor_id' => $_REQUEST['org_id'],
                ));
            }
        } else {
    ...
    private function _alipayPayFinish(&$ret) {
        ...
        if ($result && $affectedRows >= 1) {
            ...
            $ret['channel'] = $newPaymentsInfo['channel'];
            if (!$newPaymentsInfo['channel']) {
