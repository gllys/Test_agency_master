	...
	background-color: #f6fafd
				...
				<div class="form-group" style="width:200px">
					...
					<div class="ckbox ckbox-primary pull-left">
						<input type="checkbox" name="ticket_type" value="0" id="checkboxPrimary1" <?= ($param['is_union'] == -1 || $param['is_union'] == 0) ? 'checked' : '' ?>> <label for="checkboxPrimary1">单票</label>
					</div>
					<div class="ckbox ckbox-primary pull-left">
						<input type="checkbox" name="ticket_type" value="1" id="checkboxPrimary" <?= ($param['is_union'] == -1 || $param['is_union'] == 1) ? 'checked' : '' ?>> <label for="checkboxPrimary">联票</label>
					</div>
				</div>
				...
				<div class="form-group">
					<button class="btn btn-primary btn-sm" type="button" id="repass_sub">搜索</button>
				</div>
			</form>
		</div>
		<!-- panel-body -->
			...
			<table class="table table-bordered mb30" id="tablecss">
				...
				<tbody>
					...
					<tr>
						...
						<td style="text-align: right" class="text-success"><?= number_format($model['group_price'], 2) ?></td>
						...
						<td><?= $model['type'] ? '任务单' : '电子票' ?></td>
							...
							<div style="position:absolute;right:2px;" class="pull-right" data-id="<?php echo $model['id'] ?>" data-fat="<?= $model['fat_price'] ?>" data-group="<?= $model['group_price']?>"><?php
...
<div class="modal-dialog modal-lg" id="modalalert" style="width: 500px; display: none;">
	<div class="modal-content">
		...
		<div class="modal-body">
			<div class="panel panel-default">
				<div class="panel-heading">
					<span style="font-size: 18px; line-height: 30px;">请等待景区确认！</span><br /> <span style="font-size: 18px; line-height: 30px;">景区确认后，您会收到一条消息提醒，请注意查收！</span>
				</div>
			</div>
		</div>
...
function buy(id,supplier_id){
     ...
     //$('#verify-modal-buy').html('');
        $.get('/ticket/buy/?price_type=1&id='+id+'&supplier_id='+supplier_id, function(data) {
            $('#verify-modal-buy').html(data);
        });
}

$("#repass_sub").click(function(){
    jqname = $('input[name="jqname"]').val(); 
    var checked = $('input[name="ticket_type"]:checked');
	var is_union = checked.length == 1 ? checked.val() : -1;
    var url = "/ticket/group?province_id="+pid+"&jqname="+jqname+"&type="+typ+"&is_union="+is_union;
...
function province(provinceid){
     pid = provinceid; 
    var url = "/ticket/group?province_id="+pid+"&jqname="+jqname+"&type="+typ;
   window.location.href = url;
...
function getLandspace(id){
    scenic = id; 
    var url = "/ticket/group?province_id="+pid+"&jqname="+jqname+"&type="+typ+"&scenic_id="+scenic;
    window.location.href = url;    
}
