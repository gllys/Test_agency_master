            ...
            if ($orderInfo) {
                ...
                if ($validateTool->validate('mobile', $ownerMobile)) {
                    ...
                    if ($orderInfo['buyer_organization']['sms_to_buyer'] == "yes") {
                       // $sendResult = $this->sendSMS($ownerMobile, $content);
                        $sendResult = SendMessage::api()->send(array('mobile'=>$ownerMobile,'content'=>$content,'type'=>1,'order_id'=>$orderInfo['id']));
                        if (!$sendResult) {
                            $msg = '短信发送失败！';
                            return false;
                        } else {
                ...
                if ($validateTool->validate('mobile', $agencyMobile)) {
                    ...
                    if ($orderInfo['buyer_organization']['sms_to_agency'] == "yes") {
                        ...
                        $sendResult = SendMessage::api()->send(array('mobile'=>$agencyMobile,'content'=>$content));
                        if (!$sendResult) {
                            $msg = '短信发送失败！';
                            return false;
                        }
    ...
    public function sendPaymentMsg($payment_id) {
        ...
        if ($result['code'] == 'succ') {
            ...
            $result = Order::api()->lists(array(
                ...
                'ids' => $order_ids,
                'fields' => 'id,name,nums,use_day,distributor_id,distributor_name,owner_mobile,owner_name,send_sms_nums,landscape_ids,supplier_id,message_open',
                ), 0);
            if ($result['code'] == 'succ') {
                $host = Yii::app()->getRequest()->getHostInfo();

                foreach ($result['body']['data'] as $order) {
                    if(!$order['message_open']){
                        continue;
                    }
                    $order['host'] = $host;
                    $content = $this->_getCreateOrderContent($order);
                   // $rs = $this->sendSMS($order['owner_mobile'], $content);
                   $rs = SendMessage::api()->send(array('mobile'=>$order['owner_mobile'],'content'=>$content,'type'=>1,'order_id'=>$order['id']));
                    if ($rs) {
