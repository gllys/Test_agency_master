...
*/
Yii::import("common.extensions.CDbCommandExt");
...
class CDbConnectionExt extends CDbConnection
{
	...
	*/
	...
	private $_pdoShardMaster = array();//分库的时的主库
	private $_pdoShardSlave = array();//分库时的从库
	private $_pdoMain = null;//模板库
	...
	*/
	protected function open($connectionString='', $username='', $password='')
	{
		...
		catch(PDOException $e)
		{
            ...
            $message = array('DB Error Info:|ServerIP:'.$_SERVER['SERVER_ADDR'].'|'.$e->getMessage().'|dbString:'.$connectionString.'|time:'.date("Y-m-d H:i:s"),time());
            $dc->send_log('DBErr',$message);
            //send log end
			if(YII_DEBUG)
			{
	...
	*/
	protected function createPdoInstance()
	{
		//兼容5.2.x @aliang
		$args = func_get_args();
		list($connectionString, $username, $password) = $args;
		$pdoClass='PDO';
	...
	*/
	...
	public function setShardValue($shardValue){
		$this->_shardValue = $shardValue;
		$this->getPdoInstance();
		return $this;
	}
	...
	*/
	public function getPdoInstance($useSlave=false)
	{
		...
		{//从库
			if($this->dbShardType!=null){//有分库逻辑
				if($this->shardValue ===null ){//未设置分库变量，选择模板库
					if($this->_pdoMain)
						$this->_pdo = $this->_pdoMain;
					else
						$this->_pdo = $this->_pdoMain = $this->open($this->connectionString, $this->username, $this->password);
				}else{
					$shardManager = $this->getShardManager($this->dbShardType);
					$dynamicName = $shardManager->calculateDynamicName($this->shardValue);	
					
					if(isset($this->_pdoShardSlave[$dynamicName]))
						$this->_pdo = $this->_pdoShardSlave[$dynamicName];
					else{	
						$randIndex = array_rand($this->slaveConfig);
						$hostConfig = $this->slaveConfig[$randIndex];
						$params = $this->getDbParams($hostConfig, $dynamicName);
						if($params == null ) throw new CDbException(Yii::t('yii','CDbConnection cannot select a DB config1.'));
						extract($params);
						$this->_pdo = $this->_pdoShardSlave[$dynamicName] = $this->open($connectionString, $username, $password);
					}
				}
			}else{//无分库逻辑
				if($this->_pdoSlave)
					...
					$randIndex = array_rand($this->slaveConfig);
					$params = $this->slaveConfig[$randIndex];
					extract($params);
			...
			if($this->dbShardType!=null){//有分库逻辑
				if($this->shardValue ===null ){//未设置分库变量，选择模板库
					if($this->_pdoMain)
						...
						$this->_pdo = $this->_pdoMain = $this->open($this->connectionString, $this->username, $this->password);
				}else{//设置了分库变量，按分库变量选择一个主库	
					$shardManager = $this->getShardManager($this->dbShardType);
					$dynamicName = $shardManager->calculateDynamicName($this->shardValue);	
					
					if(isset($this->_pdoShardMaster[$dynamicName]))
						$this->_pdo = $this->_pdoShardMaster[$dynamicName];
					else{	
						$hostConfig = $this->masterConfig;
						$params = $this->getDbParams($hostConfig, $dynamicName);
						if($params == null ) throw new CDbException(Yii::t('yii','CDbConnection cannot select a DB config2.config:'.var_export($hostConfig,true).",dynamicName=".var_export($dynamicName, true)));
						extract($params);
						$this->_pdo = $this->_pdoShardMaster[$dynamicName] = $this->open($connectionString, $username, $password);
					}
				}
			}else{//无分库逻辑,选择主库
				if($this->_pdoMaster)
					...
					$this->_pdo = $this->_pdoMaster = $this->open($this->connectionString, $this->username, $this->password);
			}
		}
		return $this->_pdo;
	}
	...
	protected function getDbParams($hosts, $dynamicName){
		$config = null;
		...
		$dbName =  $this->baseDbName."_".$dynamicName;
		$connectionString = $config['connectionString'].";dbname=".$dbName;
		return array(
